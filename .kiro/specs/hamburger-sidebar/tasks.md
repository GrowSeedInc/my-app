...
- [x] 1. (P) Stimulus サイドバーコントローラーの実装
- [x] 1.1 開閉トグル・localStorage 永続化・aria-expanded 管理を実装する
...
- [x] 1.2 click-outside 検出とスクリムクリック処理を実装する
...
- [x] 1.3 モバイル・デスクトップのデフォルト状態を初期化時に正しく設定する
...
- [x] 2. (P) レイアウト HTML の再構成
- [x] 2.1 上部ヘッダーを廃止し、サイドバー + メインコンテンツの 2 カラム構造を構築する
...
- [x] 2.2 ナビゲーション・ユーザー情報・ログアウトをサイドバーパネルに移設する
...
- [x] 2.3 レスポンシブ対応・Tailwind CSS スタイリングとアニメーションを設定する
...
- [x] 3. テストと動作確認
- [x] 3.1 サイドバーコントローラーの開閉ロジックをシステムテストで検証する（devcontainer に Chrome 未常駐のため削除。開閉動作はブラウザ手動確認で代替）
...
- [x]* 3.2 Request spec でロール別 HTML 出力とアクセス制御を検証する
...
- [x] 3.3 モバイル画面でのレスポンシブ動作と開閉アニメーションを確認する（devcontainer に Chrome 未常駐のため削除。モバイル動作はブラウザ手動確認で代替）
