...
- [ ] 2. Category・Equipment モデルに階層ロジックを追加する

- [ ] 2.1 Category モデルに self-referential アソシエーションと level enum を追加する
...
- [ ] 2.2 Category モデルにバリデーションと削除保護を追加する
...
- [ ] 2.3 (P) Equipment モデルに小分類バリデーションを追加する
...
- [ ] 3. CategoryService を level・parent_id を扱える CRUD ロジックへ更新する

- [ ] 3.1 CategoryService の create・update・destroy を階層対応にする
...
- [ ] 4. 大/中/小分類の管理者 CRUD 画面・ルート・Policy を実装する

- [ ] 4.1 (P) Admin::CategoryMajorsController と大分類管理画面を実装する
...
- [ ] 4.2 (P) Admin::CategoryMediumsController と中分類管理画面を実装する
...
- [ ] 4.3 (P) Admin::CategoryMinorsController と小分類管理画面を実装する
...
- [ ] 4.4 ルーティングと既存 CategoriesController を更新する
...
- [ ] 4.5 (P) CategoryPolicy を階層対応に拡張する
...
- [ ] 5. Stimulus 連動セレクトコントローラと備品フォームへの組み込みを実装する

- [ ] 5.1 CategorySelectController（Stimulus）を実装する
...
- [ ] 5.2 備品登録・編集フォームに3段階連動セレクトを組み込む
...
- [ ] 6. SearchService と InventoryService・備品一覧画面を階層対応に更新する

- [ ] 6.1 SearchService の備品検索に階層フィルタを追加する
...
- [ ] 6.3 (P) InventoryService のダッシュボード集計を大分類単位に変更する
...
- [ ] 7. 既存カテゴリデータを3階層に移行するマイグレーションを実装する

- [ ] 7.1 データ移行マイグレーションをマーキング方式で実装する
...
- [ ] 8. CSV エクスポート・インポートを3カラム形式に対応させる

- [ ] 8.1 (P) CsvExportService のカテゴリ出力を3カラム形式に更新する
...
- [ ] 8.2 (P) CsvImportService のカテゴリインポートを3カラム形式に更新する
...
- [ ] 9.1 (P) Category モデルと CategoryService のユニットテストを実装する
...
- [ ] 9.2 (P) 管理者コントローラの HTTP テストを実装する
...
- [ ] 9.3 (P) SearchService・InventoryService・CSV サービスのテストを実装する
...
- [ ] 9.4 カテゴリ階層→備品登録→検索→削除の統合テストを実装する
